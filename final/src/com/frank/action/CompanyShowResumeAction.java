...
import com.frank.model.ApplicantDAO;
 ...
 * 
 */
public class CompanyShowResumeAction extends ActionSupport {
	private String name;
	...
	@Action(value = "companyShowResume", results = {
			@Result(name = ERROR, location = "/show/error.jsp"),
			@Result(name = SUCCESS, location = "/company/showResume.jsp") })
	...
	private boolean getData() {
		...
		List<Resume> list = dao.findByUserName(name);
		...
		try{
		   ...
		   request.setAttribute("companyResumeList", list);
		}catch(Exception e){
			e.printStackTrace();
			flag=false;
		}
